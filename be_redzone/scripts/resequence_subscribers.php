...
// CLI only. Default: rehearse against temporary tables. --apply changes the local DB.
if (PHP_SAPI !== 'cli') {
...
try {
    ...
    $mapping = [];
    foreach ($snapshot['subscribers'] as $index => $row) {
        $mapping[(int) $row['id']] = $index + 1;
    }
    ...
    if ($apply) {
        ...
        $backupDir = sys_get_temp_dir().'/redzone-resequence-'.date('Ymd-His').'-'.bin2hex(random_bytes(4));
    ...
    foreach ($snapshot['subscriptions'] as $row) {
        $db->table($child)->where('id', $row['id'])->update(['subscriber_id' => $mapping[(int) $row['subscriber_id']]]);
    }
    ...
    if ($apply) {
        ...
        $db->statement('ALTER TABLE `subscribers` AUTO_INCREMENT='.(count($mapping) + 1));
    } else {
        $db->rollBack();
    }
    $report = ['mode' => $apply ? 'applied' : 'rehearsal', 'subscribers' => count($mapping),
        ...
        'new_maximum' => count($mapping), 'next_id' => count($mapping) + 1,
